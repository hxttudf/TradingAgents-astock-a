...
def create_fundamentals_analyst(llm):
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "\n1. PE（TTM）、PB、总市值（从 get_fundamentals 获取）"
            "\n2. 营收同比增长率（调用 get_income_statement 获取利润表后计算）"
            "\n3. 归母净利润及同比增长率（从 get_income_statement 获取）"
            "\n4. 扣非净利润（调用 get_income_statement 获取利润表中的「扣非净利润」字段）"
            "\n5. 毛利率 = (营业收入 - 营业成本) / 营业收入（从 get_income_statement 获取后计算）"
            "\n6. 营业利润率 = 营业利润 / 营业收入（从 get_income_statement 获取后计算）"
            "\n7. ROE（调用 get_balance_sheet + get_income_statement 获取后计算）"
            "\n8. 资产负债率 = 总负债 / 总资产（调用 get_balance_sheet 获取后计算）"
            "\n9. 商誉（调用 get_balance_sheet 获取资产负债表中的「商誉」字段）"
            "\n10. 经营性现金流与净利润比值（调用 get_cashflow 获取后计算）"
            "\n11. 机构一致预期 EPS（调用 get_profit_forecast 获取）"
            + get_language_instruction()
        )
